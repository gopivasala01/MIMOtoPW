...
public class AppConfig 
{
	 public static boolean saveButtonOnAndOff= true;
	  ...
	  // public static String leaseFetchQuery  = "Select Company, Building,leaseName from Automation.InitialRentsUpdate where Status ='Pending' and Company ='Georgia'";
	   ...
	   public static String pendingLeasesQuery = "Drop table #Temp\n"
	   		+ "SELECT ID,\n"
	   		+ "    Unit_Entity_ID,\n"
	   		...
	   		+ "    AutomationStatus,\n"
	   		+ "	AsOfDate,\n"
	   		+ "	Note,RowRank = ROW_NUMBER() OVER(order by ID ) INTO #Temp FROM Automation.MIMOToPw_Prod\n"
	   		...
	   		+ "Order by 1 asc \n"
	   		+ "\n"
	   		+ "Declare @Count INT  = (SELECT COUNT(RowRank)/3 FROM #Temp)\n"
	   		+ "DECLARE @Count2 INT  = @Count * 2\n"
	   		+ "DECLARE @Count3 INT  = @Count * 3\n"
	   		+ "DECLARE @MAX INT  = (SELECT MAX(RowRank) FROM #Temp)\n"
	   		+ "--1St one table --\n"
	   		...
	   		+ "--2Nd One Table --\n"
	   		+ "SELECT * FROM #Temp\n"
	   		+ "Where RowRank Between @Count+1 AND @Count2\n"
	   		+ "Order by 1 asc\n"
	   		+ " \n"
	   		+ "--3Rd One Table --\n"
	   		+ "--SELECT * FROM #Temp\n"
	   		+ "--Where RowRank BETWEEN @Count2+1 AND @MAX\n"
	   		+ "--Order by 1 asc\n"
	   		+ ""
		   		+ ""
				    ;
